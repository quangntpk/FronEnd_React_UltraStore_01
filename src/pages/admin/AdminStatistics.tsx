...
const Dashboard = () => {
  ...
  const getOrderStatusStatistics = async () => {
    ...
    if (timeFilter === "daily" && dailyDate) {
      const [year, month, day] = dailyDate.split("-");
      url += `?year=${year}&month=${month}&day=${day}`;
    } else if (timeFilter === "monthly" && monthlyYear && monthlyMonth) {
      url += `?year=${monthlyYear}&month=${monthlyMonth}`;
    } else if (timeFilter === "yearly" && yearlyYear) {
      url += `?year=${yearlyYear}`;
    }
    setIsLoading(true);
  ...
  const getTopProductsStatistics = async () => {
    ...
    try {
      ...
      const data = await response.json();
      setTopProductsData(data.length > 0 ? data : []);
    } catch (error) {
                ...
                <CardContent>
                  <div className="flex items-center gap-4 mb-4">
                    <input
                      ...
                      aria-label="Chọn ngày thống kê"
                    />
                    <Button onClick={getOrderStatusStatistics}>Thống kê</Button>
                  </div>
