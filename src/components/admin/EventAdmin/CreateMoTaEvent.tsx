...
const KhuyenMaiForm = ({ onCancel }) => {
  const [formData, setFormData] = useState({
    ...
    MoTa: {
      ...
      Pictures: [], // Store { file: File, preview: string }
      Title: [{
        Name: '',
        Subtitle: [{
          Name: '',
          ...
          Picture: {
            Url: null // Store { file: File, preview: string } or null
          }
        }],
        Picture: {
          Url: null // Store { file: File, preview: string } or null
  ...
  // Clean up object URLs on unmount or form reset
  useEffect(() => {
    return () => {
  ...
  // Fixed updateNestedValue function to preserve File objects
  const updateNestedValue = (path, value) => {
    const pathArray = path.split('.');
    
    setFormData(prevFormData => {
      // Create a deep clone that preserves File objects
      const deepClone = (obj) => {
  ...
  const handleDrop = (e, path, multiple = false) => {
    ...
    if (multiple) {
      ...
      setFormData(prevFormData => ({
        ...
        MoTa: {
          ...prevFormData.MoTa,
          Pictures: [...prevFormData.MoTa.Pictures, ...newPictures]
        }
      }));
    } else {
      if (files.length > 0) {
  ...
  const removePicture = (index) => {
    const picture = formData.MoTa.Pictures[index];
    if (picture.preview) {
      URL.revokeObjectURL(picture.preview);
      previewUrls.current.delete(picture.preview);
    }
    const newPictures = formData.MoTa.Pictures.filter((_, i) => i !== index);
    setFormData(prevFormData => ({
      ...prevFormData,
      MoTa: {
        ...prevFormData.MoTa,
  ...
  const addTitle = () => {
    ...
    setSelectedTitleIndex(formData.MoTa.Title.length);
  ...
  const addSubtitle = (titleIndex) => {
    ...
    setSelectedSubtitleIndex(formData.MoTa.Title[titleIndex].Subtitle.length);
  };
  ...
  // Helper function to convert File to base64 (binary data only)
  const fileToBase64 = (file) => {
    return new Promise((resolve, reject) => {
      const reader = new FileReader();
      reader.readAsDataURL(file);
      reader.onload = () => {
        // Remove data:image/jpeg;base64, prefix, only keep binary data
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Convert all images to base64 (binary data only)
      const convertedPictures = [];
      for (const pic of formData.MoTa.Pictures) {
      ...
      // Convert title pictures and subtitle pictures
      ...
      // Prepare final data according to MoTaKhuyenMai structure
      // IMPORTANT: Use "Picture" (not "Pictures") because of [JsonPropertyName("Picture")] attribute
      const moTaData = {
        Header: {
          Title: formData.MoTa.Header.Title
        },
        Picture: convertedPictures, // Changed from "Pictures" to "Picture"
        Title: convertedTitles
      };

      console.log('Sending data to API:', {
        ...moTaData,
        Picture: moTaData.Picture ? moTaData.Picture.map((pic, index) => ({
          ...pic,
      ...
      // Send to backend
      const response = await fetch('http://localhost:5261/api/KhuyenMai/MoTaKhuyenMaiCreate', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to save MoTaKhuyenMai: ${response.status} - ${errorText}`);
      }

      const result = await response.json();
      console.log('MoTaKhuyenMai saved successfully:', result);
      ...
      setSelectedSubtitleIndex(0);
      alert('Mô tả khuyến mãi đã được lưu thành công!');
      onCancel(); // Close the form
    } catch (error) {
      console.error('Error saving MoTaKhuyenMai:', error);
      setError(error.message);
      alert(`Lỗi khi lưu mô tả khuyến mãi: ${error.message}`);
    } finally {
      ...
      <div className="container mx-auto p-6">
        <div className="bg-white rounded-2xl shadow-xl p-8 mb-8 border border-gray-100">
          ...
          <div>
            ...
            {formData.MoTa.Pictures.length > 0 && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
                {formData.MoTa.Pictures.map((picture, index) => (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              {formData.MoTa.Title.map((title, index) => (
                ...
                >
                  ...
                  <div
                    ...
                    onClick={(e) => e.stopPropagation()}
                  >
                    {title.Picture.Url?.file instanceof File ? (
                      <div className="relative">
                        <img src={title.Picture.Url.preview} alt="Title preview" className="w-full h-20 object-cover rounded-lg" />
                        <button
                          type="button"
                          onClick={(e) => {
                            e.stopPropagation();
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6 border border-gray-100">
            ...
            {formData.MoTa.Title[selectedTitleIndex] && (
              <div className="space-y-4 max-h-96 overflow-y-auto pr-2">
                {formData.MoTa.Title[selectedTitleIndex].Subtitle.map((subtitle, index) => (
                  ...
                  >
                    ...
                    >
                      {subtitle.Picture.Url?.file instanceof File ? (
                        <div className="relative">
                          <img src={subtitle.Picture.Url.preview} alt="Subtitle preview" className="w-full h-16 object-cover rounded-lg" />
                          <button
                            type="button"
        ...
        <div className="text-center mt-8 flex justify-center gap-4">
          ...
          >
            {isSubmitting ? 'Đang lưu...' : 'Lưu mô tả'}
          </button>
        </div>
      </div>
    </div>
