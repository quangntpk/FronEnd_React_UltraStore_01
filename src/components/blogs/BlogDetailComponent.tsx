...
export const BlogDetailComponent = () => {
  ...
  const fetchBlog = async () => {
    try {
      setLoading(true);
      console.log("Fetching from:", `${import.meta.env.VITE_API_URL}/api/Blog/slug/${slug}`); // Debug URL
      const blogResponse = await axios.get<Blog>(`${import.meta.env.VITE_API_URL}/api/Blog/slug/${slug}`); // Sử dụng /slug/
      console.log("Data received:", blogResponse.data); // Debug dữ liệu
      setBlog(blogResponse.data);

      try {
        const nguoiDungResponse = await axios.get<NguoiDung>(`${import.meta.env.VITE_API_URL}/api/NguoiDung/${blogResponse.data.maNguoiDung}`);
  ...
  const roleBackground = nguoiDung?.vaiTro === 1 ? "bg-green-500" : nguoiDung?.vaiTro === 2 ? "bg-green-500" : "bg-green-500";
      ...
      <Card className="max-w-3xl mx-auto">
        {blog.hinhAnh && (
          <img
            ...
            className="w-full h-64 sm:h-96 object-cover rounded-t-md"
          />
        )}
        <CardHeader>
          <CardTitle className={`${isMobile ? "text-2xl" : "text-3xl"} font-bold`}>{blog.tieuDe}</CardTitle>
          
        </CardHeader>
        <CardContent>
          <div className="prose prose-sm sm:prose lg:prose-lg max-w-none" dangerouslySetInnerHTML={{ __html: blog.noiDung }} />
          <div className="flex justify-between items-center mt-4 text-sm text-gray-500">
            <div>
              <span>Người tạo: {nguoiDung?.hoTen || blog.maNguoiDung}</span>
              {nguoiDung?.vaiTro && (
                <span className={`ml-2 px-2 py-1 rounded text-white ${roleBackground}`}>{roleText}</span>
              )}
            </div>
