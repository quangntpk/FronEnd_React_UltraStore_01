...
const GapThu: React.FC = () => {
  ...
  // Đặt hàm pop trước pickup để khắc phục lỗi TS2448
  const pop = useCallback((): void => {
    ...
    if (titleRef.current) {
      ...
      if (titleElement && selectedVoucher) {
        // Có thể thêm nội dung tiêu đề nếu cần
      }
    }

    // Hiển thị modal ngay lập tức thay vì sau 2 giây
    setShowWinnerPopup(true);
  }, [prizeBall, selectedVoucher]);
  ...
  const pickup = useCallback(async (): Promise<void> => {
    ...
    // Tăng giá trị này để di chuyển sang bên phải nhiều hơn
    const rightOffset = 24;
    const dropX = x + rightOffset;
    
    // Điều chỉnh điểm bắt đầu rơi thấp hơn
    const startDropY = y + 30; // Hạ thấp điểm bắt đầu xuống 30vh
    ...
    prizeBall.dom.style.transform = `translate(${dropX}vh, ${startDropY}vh) rotate(${prizeBall.rotate}deg)`;
    ...
    setTimeout(() => {
      if (prizeBall.dom) {
        prizeBall.dom.style.transform = `translate(${dropX}vh, 50vh) rotate(${prizeBall.rotate - 180}deg)`;
      }
    }, 100);

    setTimeout(() => {
      if (prizeBall.dom) {
        prizeBall.dom.style.transform = `translate(${dropX}vh, 52vh) rotate(${prizeBall.rotate - 180}deg)`;
        setTimeout(() => {
          if (prizeBall.dom) {
            prizeBall.dom.style.transform = `translate(${dropX}vh, 50vh) rotate(${prizeBall.rotate - 180}deg)`;
            setTimeout(() => {
              if (prizeBall.dom) {
                prizeBall.dom.style.transform = `translate(${dropX}vh, 53vh) rotate(${prizeBall.rotate - 180}deg)`;
                setTimeout(() => {
                  if (prizeBall.dom) {
                    prizeBall.dom.style.transform = `translate(${dropX}vh, 50vh) rotate(${prizeBall.rotate - 180}deg)`;
                    setTimeout(() => {
  ...
  // Sửa hàm fetchVouchers để khắc phục lỗi TS1005, TS2304, TS2345
  const fetchVouchers = useCallback(async (): Promise<void> => {
    try {
      ...
      const validVouchers = data
        .filter((voucher) => {
          const startDate = new Date(voucher.ngayBatDau); // Xóa Ascending
                    ...
                    setTimeout(() => {
                      if (prizeBall?.dom) {
                        ...
                        setTimeout(() => {
                          if (prizeBall?.dom) {
                            prizeBall.dom.style.transform = `translateY(30vh) translateX(-3vh) rotate(${prizeBall.rotate + 10}deg)`;
                            prizeBall.dom.style.pointerEvents = "auto";
                            
                            // Tự động gọi pickup thay vì chờ click
                            pickup();
